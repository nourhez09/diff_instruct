...
import click
import dnnlib
from training import di_training_loop2 as training_loop
...
def main(**kwargs):
    ...
    # Description string for run.
    cond_str = 'cond' if c.dataset_kwargs.use_labels else 'uncond'
    dtype_str = 'fp16' if c.network_kwargs.use_fp16 else 'fp32'
    desc = f'{dataset_name:s}-{cond_str:s}-ls{opts.ls}-sgls{opts.sgls}-glr{opts.glr}-sglr{opts.lr}-sigma{opts.init_sigma}-batch{c.batch_size:d}-{dtype_str:s}-lrwarmkimg{opts.lr_warmup_kimg}'
